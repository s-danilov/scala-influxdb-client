...
class Database protected[influxdbclient] (val databaseName: String, httpClient: HttpClient)(
  ...
  private def executeWrite(payload:         String,
                           precision:       Precision,
                           consistency:     Consistency,
                           retentionPolicy: String): Future[Boolean] = {
    val params = buildWriteParameters(databaseName, precision, consistency, retentionPolicy)

    httpClient
      .post("/write", params, payload)
      .recover {
        case error: HttpException =>
          throw exceptionFromStatusCode(error.code, "Error during write: " + error.getMessage, error)
      }
      .map { result =>
        if (result.code != 204)
  ...
  private def buildWriteParameters(databaseName:    String,
    ...
    val params =
      List(("db", databaseName), ("precision", precision), ("consistency", consistency), ("rp", retentionPolicy))
